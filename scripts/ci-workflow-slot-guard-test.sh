...
#
# Mirrors AskExe/exe-os tests/ci/ci-workflow-slot-guard-test.sh,
# AskExe/exe-wiki and AskExe/exe-erp scripts/ci-workflow-slot-guard*.
# Asserts that .github/workflows/ci.yml's server-tests job still
...
# restructured to stop calling a script while the script itself keeps
# passing its own tests (this is bug d859577c's pattern: a workflow guard
# regressed silently via restructuring). Pure text assertions, no
# yq/python dependency.

set -euo pipefail

WORKFLOW="${CI_WORKFLOW_GUARD_TEST_FILE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/.github/workflows/ci.yml}"
...
if grep -q 'run: bash scripts/ci-concurrency-slot.sh$' "$WORKFLOW"; then
  pass "server-tests job acquires from scripts/ci-concurrency-slot.sh"
else
...
if grep -q 'run: bash scripts/ci-concurrency-slot.sh --release' "$WORKFLOW"; then
  pass "server-tests job releases via 'ci-concurrency-slot.sh --release'"
else
...
if grep -B10 -- '--release' "$WORKFLOW" | grep -q 'if: always()'; then
  pass "release step is guarded with if: always() (retained even alongside the dependabot skip condition)"
else
...
if grep -qE '^\s*CI_HEAVY_SLOT_WORKERS:\s*"' "$WORKFLOW"; then
  pass "server-tests job declares CI_HEAVY_SLOT_WORKERS"
else
