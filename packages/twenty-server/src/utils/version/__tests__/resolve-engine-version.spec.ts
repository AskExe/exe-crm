...
describe('resolveEngineVersion', () => {
  ...
  )('honors engine-track version %s as-is', (version) => {
  ...
  it.each([
    ...
    'maps %s (%s) to the current engine version',
    (_label, appVersion) => {
  ...
  ])('falls back to the current engine version when APP_VERSION is %s', (_label, appVersion) => {
    expect(resolveEngineVersion(appVersion).version).toBe(
      CURRENT_ENGINE_VERSION,
    );
  });
