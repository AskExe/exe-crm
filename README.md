...
| `EXE_LICENSE_KEY` | Customer installation key; feature checks verify it through the GoTrue-backed license gateway. |
| `EXE_LICENSE_URL` | Optional HTTPS activation endpoint (default `https://cloud.askexe.com/auth/activate`). Invalid licenses deny features; unavailable authority returns a retryable error. |
