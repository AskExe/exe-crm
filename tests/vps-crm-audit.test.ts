...
describe('exe-crm VPS regression audit', () => {
  ...
  it('entrypoint hard-gates EXE_LICENSE_KEY and rejects placeholder values', () => {
    ...
    expect(entrypoint).toContain('license_key="${EXE_LICENSE_KEY:-${ENTERPRISE_KEY:-}}"');
    expect(entrypoint).toContain('EXE_LICENSE_KEY is required. Obtain a valid key from https://askexe.com before booting Exe CRM.');
    expect(entrypoint).toMatch(/CHANGEME\*\|changeme\*\|replace_me\*\|REPLACE_ME\*\|your_\*\|YOUR_\*\|example\*\|EXAMPLE\*/);
    expect(entrypoint).toContain('export EXE_LICENSE_KEY="$license_key"');
    expect(entrypoint).toContain('export ENTERPRISE_KEY="${ENTERPRISE_KEY:-$license_key}"');
    expect(entrypoint).toMatch(/assert_exe_license_key\s*\nsetup_and_migrate_db\s*\nregister_background_jobs/);
  });

  it('main.ts boot path asserts EXE_LICENSE_KEY before Nest startup', () => {
    expect(mainTs).toContain('const assertExeLicenseKey = () => {');
    expect(mainTs).toContain('const licenseKey = process.env.EXE_LICENSE_KEY ?? process.env.ENTERPRISE_KEY;');
    expect(mainTs).toContain("/^(changeme|replace_me|your_|example)/i.test(value)");
    expect(mainTs).toContain('EXE_LICENSE_KEY must be set to a real enterprise key before Exe CRM can boot.');
    expect(mainTs).toContain('process.env.EXE_LICENSE_KEY = licenseKey;');
    expect(mainTs).toContain('process.env.ENTERPRISE_KEY = process.env.ENTERPRISE_KEY ?? licenseKey;');
    expect(mainTs).toMatch(/const bootstrap = async \(\) => \{[\s\S]*assertAppSecret\(\);\s*assertExeLicenseKey\(\);/);
  });

  it('docker compose stays parseable after required-var quoting fix', () => {
    const composePath = path.join(ROOT, 'packages/twenty-docker/docker-compose.yml');
    ...
    try {
      ...
      const error = err as NodeJS.ErrnoException & { stderr?: Buffer; stdout?: Buffer };
      if (error.code === 'ENOENT') {
        expect(compose).toContain('APP_SECRET: "${APP_SECRET:?APP_SECRET is required - generate with openssl rand -base64 32}"');
        expect(compose).toContain('EXE_LICENSE_KEY: "${EXE_LICENSE_KEY:?EXE_LICENSE_KEY is required - purchase at https://askexe.com}"');
        return;
      }
      throw new Error(
  ...
  it('publishes CRM only on localhost with configurable host port', () => {
    expect(compose).toContain('- "127.0.0.1:${CRM_HOST_PORT:-3000}:3000"');
  });

  it('requires EXE_LICENSE_KEY in both server and worker env blocks', () => {
    const matches = compose.match(/EXE_LICENSE_KEY: "\$\{EXE_LICENSE_KEY:\?EXE_LICENSE_KEY is required - purchase at https:\/\/askexe\.com\}"/g);
    expect(matches?.length).toBe(2);
  });

  it('GoTrue JWT bridge keeps the expected validation imports and helpers', () => {
    expect(accessTokenService).toContain("import { createPublicKey } from 'crypto';");
    expect(accessTokenService).toContain("import * as jwt from 'jsonwebtoken';");
    expect(accessTokenService).toContain('type GoTrueJwtPayload = jwt.JwtPayload & {');
    ...
    expect(accessTokenService).toContain('const verified = jwt.verify(token, this.getGoTrueVerificationKey(jwk), {');
  });
});
